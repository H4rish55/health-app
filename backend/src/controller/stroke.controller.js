...
const stroke = async (req, res) => {
    try {
        ...
        if(!gender || !age || !hypertension || !heart_disease || !ever_married || !work_type || !residence_type || !avg_glucose_level || !bmi || !smoking_status ){
        ...
        if(hypertensionLower !== "yes" && hypertensionLower !== "no"){
            return res.status(400).json({ success: false, message: "Hypertension must be Yes or No" })
        }

        if(heartDiseaseLower !== "yes" && heartDiseaseLower !== "no"){
            return res.status(400).json({ success: false, message: "Heart disease must be Yes or No" })
        }
        
        if(everMarriedLower !== "yes" && everMarriedLower!== "no"){
            return res.status(400).json({ success: false, message: "Ever married must be Yes or No" })
        }

        if(work_type !== "Private" && work_type !== "Self-employed" && work_type !== "Govt_job"){
            return res.status(400).json({ success: false, message: "Invalid work type" })
        }

        if(residence_type !== "Urban" && residence_type !== "Rural"){
            return res.status(400).json({ success: false, message: 'Invalid resident type' })
        }

        if(avg_glucose_level < 0 || avg_glucose_level > 500){
            return res.status(400).json({ success: false, message: 'Invalid glucose level' })
        }

        if(bmi < 0 || bmi > 250){
            return res.status(400).json({ success: false, message: 'Invalid bmi range' })
        }

        if(smoking_status !== 'formerly smoked' && smoking_status !== 'never smoked' && smoking_status !== 'smokes' && smoking_status !== 'Unknown'){
            return res.status(400).json({ success: false, message: 'Invalid smoking status' })
        }
